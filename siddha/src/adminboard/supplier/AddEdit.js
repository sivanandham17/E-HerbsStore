...
const AddEdit = () => {
    ...
    const {name, company, email, contact, stprice} = state;
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if(!name || !company || !email || !contact || !stprice)
        {
          ...
          if(!id){
            axios.post('http://localhost:5000/api/post/supplier',{
                name,company,email,contact,stprice,
            }).then(() => {
                setState({name: '', company: '', email: '', contact: '' ,stprice: ''});
            })
  ...
  return (
    ...
    <div className='sadminform'>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input className='ipt' type='text' id='company' name='company' placeholder='COMPANY' value={company || ""} onChange={handleInputChange}/><br/>
