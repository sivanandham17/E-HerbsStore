...
const AddEdit = () => {
    ...
    const {email,pdtname, category, quantity, date, price} = state;
        ...
        if(!pdtname || !category )
        {
          ...
          if(!id){
            axios.post('http://localhost:5000/api/post/order',{
                email, pdtname, category, quantity, picture, date, price, 
            }).then(() => {
                setState({email:'',pdtname: '', category: '', quantity: '', pdtimage: '', date: '', price: ''});
            })
  ...
  return (
    ...
    <div className='adminuserform'>
      ...
      <label>Email</label><br/>
        ...
        <input className='ipt' type='number' id='price' name='price' placeholder='SELLING PRICE' value={price||""} onChange={handleInputChange}/><br/><br/>
