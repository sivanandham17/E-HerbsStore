...
const AddEdit = ({supp}) => {
  ...
  const { supplierid, productname, category, quantity, description, date, price} = state;
    ...
    if (!productname || !category) {
      ...
      if (!id) {
        axios.post('http://localhost:5000/api/post/adproduct', {
          supplierid, productname, category, quantity, description, picture, date, price, 
        }).then(() => {
          setState({ supplierid: '', productname: '', category: '', quantity: '', description: '', pdtimage: '', date: '', price: '' });
        })
  ...
  return (
    <div>
      ...
      <div className='adminuserform'>
        ...
        <input className='ipt' type='number' id='price' name='price' placeholder='COST PRICE' value={price || ""} onChange={handleInputChange} /><br />
